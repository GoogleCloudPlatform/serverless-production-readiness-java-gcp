...
select * from pages where book_id=20;
-- 4. Pages Table
...
*
from
    pages where book_id=20 limit 500;
...
select * from books where title = 'The Jungle Book';
select * from bookSummaries where book_id=20;
--delete from booksummaries where book_id=20;
...
-- https://www.toolsoverflow.com/string/paragraph-to-line
...
JOIN authors a on
       a.author_id=b.author_id
WHERE b.title = 'Ulysses' AND a.name = 'James Joyce'
ORDER BY distance ASC LIMIT 10;

 SELECT
        b.title,
        left(p.content,4000) as page,
        a.name,
        p.page_number,
        (p.embedding <=> embedding('textembedding-gecko@003', 'Find the paragraphs mentioning keywords in the following list: {love,  compassion} in the book.')::vector) as distance
FROM
        pages p
JOIN books b on
        p.book_id=b.book_id
JOIN authors a on
       a.author_id=b.author_id
WHERE b.title = 'The Complete Works of William Shakespeare' AND a.name = 'William Shakespeare' ORDER BY distance ASC LIMIT 10;


 --- [cat-handler-203] services.domain.dao.DataAccess           : [4000, Find the paragraphs mentioning keywords in the following list: {love, compassion} in the book., The Complete Works of William Shakespeare, William Shakespeare]
SELECT
        b.title,
        left(p.content,4000) as page,
        a.name,
...
JOIN authors a on
       a.author_id=b.author_id
WHERE b.title = 'The Complete Works of William Shakespeare' AND a.name = 'William Shakespeare' ORDER BY
distance ASC
LIMIT 10;
select * from books;
select * from pages where book_id=10 limit 10;